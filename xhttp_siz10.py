...
async def _get_or_create_session(uuid: str, mode: str, session_id: str, ip: str = "نامشخص") -> dict:
    ...
    async with XHTTP_LOCK:
        ...
        if sess is not None:
            # یک session_id نباید بین UUID یا modeهای مختلف reuse شود؛
            # در غیر این صورت ممکن است یک کلاینت به تونل دیگری وصل شود.
            if sess.get("uuid") != uuid or sess.get("mode") != mode or sess.get("closed"):
...
async def _pump_tcp_to_queue(session_id: str, uuid: str, reader: asyncio.StreamReader, down_q: asyncio.Queue):
    ...
    try:
        ...
        logger.warning(f"XHTTP downlink pump [{session_id[:8]}] error: {exc}")
    finally:
        await gate.flush()
...
async def stream_up_upload(uuid: str, session_id: str, request: Request):
    ...
    writer = sess.get("writer")
    # request.stream() می‌تواند اولین chunk را کوچک‌تر از VLESS header بدهد.
    # بنابراین تا وقتی header کامل نشده، داده را موقتاً جمع می‌کنیم.
    ...
    try:
        async for chunk in request.stream():
            ...
            if writer is None:
                ...
                try:
                    # فقط برای تشخیص کامل‌شدن header parse می‌کنیم؛ اتصال واقعی
                    # در _open_tcp_for_session انجام می‌شود تا payload حفظ شود.
                    await _open_tcp_for_session(session_id, uuid, sess, bytes(header_buf))
                except ValueError as exc:
                    if "too small" in str(exc):
                        # header هنوز کامل نیست؛ chunk بعدی را صبر کن.
                        continue
                    raise
                writer = sess.get("writer")
                header_buf.clear()
        ...
        if writer is None and header_buf:
